"""TPO-Torch Honest Benchmark — TPO vs CE vs PPO-clip.
...
def tpo_loss(policy_logits, reference_logits, labels, advantages, beta=0.1, attention_mask=None):
...
class TinyTransformer(nn.Module):
    def __init__(self, vocab=64, d=64, seq=16):
        ...
        enc = nn.TransformerEncoderLayer(d, nhead=4, dim_feedforward=d * 2, batch_first=True)
...
def train_ppo(model, tx, ty, vx, vy, vocab=64):
    ...
    for _ in range(60):
        ...
        logits = model(tx)
        with torch.no_grad():
            old_lp = F.log_softmax(logits, -1).gather(2, ty.unsqueeze(-1)).squeeze(-1)
            rewards = (logits.argmax(-1) == ty).float()
        new_lp = F.log_softmax(logits, -1).gather(2, ty.unsqueeze(-1)).squeeze(-1)
...
def run_training(VOCAB=64, SEQ=16, N_SEEDS=3):
    print(f"[1/3] Training: TPO vs CE vs PPO-clip ({N_SEEDS} seeds)")
    tx, ty = make_data(256, SEQ, VOCAB)
    vx, vy = make_data(64, SEQ, VOCAB)

    all_ppls = {}
    for name, fn in [("CE", train_ce), ("PPO-clip", train_ppo), ("TPO", train_tpo)]:
        ppls = []
    ...
    steps = np.arange(1, len(means["CE"]) + 1)

    colors = {"CE": "#2196F3", "PPO-clip": "#4CAF50", "TPO": "#E91E63"}
    fig, ax = plt.subplots(figsize=(10, 5))
    for name in ["CE", "PPO-clip", "TPO"]:
        ax.plot(steps, means[name], label=name, color=colors[name], lw=2)
        ax.fill_between(steps, means[name] - stds[name], means[name] + stds[name],
    ...
    return {
        ...
        "ce_final": round(float(means["CE"][-1]), 2),
...
def run_stability():
    ...
    for av in advs_vals:
        ...
        for _ in range(5):
            ...
            ref = torch.randn(batch, seq, vocab)
            labels = torch.randint(0, vocab, (batch, seq))
            loss = tpo_loss(logits, ref, labels, torch.full((batch,), av), beta=0.1)
            loss.backward()
    ...
    for b in betas:
        ...
        for _ in range(5):
            ...
            ref = torch.randn(batch, seq, vocab)
            labels = torch.randint(0, vocab, (batch, seq))
            loss = tpo_loss(logits, ref, labels, torch.randn(batch) * 2, beta=b)
    ...
    for seq in seq_lengths:
        try:
            ...
            for _ in range(3):
                ...
                loss.backward(); logits.grad = None
            if device == "cuda":
...
def main():
    print("=" * 60)
    print("  TPO-Torch Benchmark (corrected math, fair comparison)")
    print("=" * 60)
    device = "cuda" if torch.cuda.is_available() else "cpu"
